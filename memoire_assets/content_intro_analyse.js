...
const introduction = [
  ...
  table(
    ...
    [
      ...
      ["Espace administrateur", "Connexion sécurisée (JWT 24 h) et dashboard permettant la gestion des projets, des articles de blog, des messages de contact et le changement de mot de passe."],
      ["API REST backend", "18 endpoints exposés sur 4 ressources (auth, projects, posts, contact), sécurisés par JWT et rôle administrateur."],
      ["Base de données", "Base MySQL de 4 tables : admin, project, blog_post et contact_message."],
      ["Envoi d'emails", "Réponse aux messages de contact via Gmail SMTP."],
    ],
    [2400, 6950]
  ),
  ...
  p("Le développement et la démonstration de l'application sont réalisés en environnement local (WAMP Server sous Windows) : aucun déploiement en ligne n'est prévu dans le cadre de ce TFE, ce point est détaillé en section 3.6."),
  ...
  p("Certaines fonctionnalités, envisagées un temps durant l'analyse, ont été volontairement écartées du périmètre livré pour rester réalisable dans les délais de la deuxième session : la synchronisation avec un calendrier externe (Google Calendar), la mise en place de notifications push, et une gestion de plusieurs administrateurs avec des droits différenciés. Ces pistes sont reprises comme perspectives d'évolution en section 4.4."),
...
const analyse = [
  ...
  table(
    ...
    [
      ...
      ["Inscriptions", "S'inscrire, rejoindre la liste d'attente si l'événement est complet, se désinscrire librement", "Bénévole"],
  ...
  p("Les inscriptions et la liste d'attente (RG-09 à RG-14) forment le cœur fonctionnel du module, détaillé pas à pas dans les scénarios de la section 2.3. Le bénévole y garde la main sur sa désinscription à tout moment avant le début de l'événement, tandis que la confirmation d'une inscription reste toujours un acte explicite de l'administrateur : le système ne confirme jamais automatiquement une première inscription, seule une promotion depuis la liste d'attente l'est (RG-13)."),
  ...
  p("Le module réunissant deux univers assez distincts (l'espace public/bénévole d'un côté, l'administration de l'autre), le diagramme de cas d'utilisation est présenté en deux volets séparés plutôt qu'en un seul schéma dense, pour une lecture plus explicite de chaque côté."),

  h3("Côté bénévole"),
  p("Le premier volet représente les interactions du Visiteur et du Bénévole avec le module. Le Bénévole étend le Visiteur par généralisation (« est un »), puisque tout bénévole reste un visiteur capable de consulter le site public. Le diagramme distingue volontairement deux blocs de cas d'utilisation reliés au même acteur : les cas accessibles dès la consultation du site (consulter le site vitrine, consulter les événements, créer un compte) et ceux qui exigent d'être connecté (gérer son profil, s'inscrire, se désinscrire, laisser un avis, télécharger une attestation). Cette distinction reflète directement la séparation des routes publiques et protégées définie dans SecurityConfig (section 3.5.2) : rien dans le diagramme ne suppose une contrainte technique qui ne soit pas également appliquée dans le code. Le cas « S'inscrire à un événement » est étendu (relation «extend») par « Rejoindre la liste d'attente », qui ne s'active que dans la situation particulière où l'événement est déjà complet : une extension optionnelle du comportement de base plutôt qu'un cas d'utilisation systématique."),
  ...imgPara(SCR + "diagramme_cas_utilisation_benevole.png", 344, 600, "Figure 1 — Diagramme de cas d'utilisation, côté bénévole"),

  h3("Côté administration"),
  p("Le second volet représente les interactions de l'Administrateur, rôle unique côté Terra Sana. Le cas « Valider / refuser les inscriptions » inclut (relation «include») systématiquement l'envoi d'un email de confirmation : ce sous-cas n'est jamais déclenché seul, il fait partie intégrante de la validation elle-même, conformément à RG-12."),
  ...imgPara(SCR + "diagramme_cas_utilisation_admin.png", 369, 600, "Figure 2 — Diagramme de cas d'utilisation, côté administration"),
  ...
  p("Ce troisième groupe est le plus dense, car il porte le cœur métier du module : garantir qu'une place n'est jamais attribuée deux fois, offrir une liste d'attente lorsque la capacité est atteinte, et automatiser autant que possible la communication avec le bénévole à chaque étape, sans jamais retirer à l'administrateur le dernier mot sur une validation."),
  ...
  rgItem("RG-19", "Le niveau du bénévole est calculé automatiquement à partir de son nombre de participations confirmées : BRONZE (0 à 2 événements), ARGENT (3 à 6), OR (7 et plus)."),
  ...
  rgItem("RG-23", "Toute action nécessitant un compte est protégée par un jeton JWT à durée limitée (24 heures pour l'administrateur, 7 jours pour le bénévole) ; l'expiration force la reconnexion."),
