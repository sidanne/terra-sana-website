...
const conclusion = [
  ...
  table(["Critère", "Stage", "TFE (apport personnel)"], [
    ...
    ["Endpoints API REST", "18", "+36 nouveaux (54 au total)"],
    ...
    ["Envois d'emails automatiques", "réponses au contact uniquement", "confirmation, liste d'attente, groupe, réinitialisation, changement de niveau"],
  ...
  p("Sur le plan humain, ce travail a aussi été une première expérience de traduction d'un besoin flou en spécification précise. Les entretiens avec Monsieur Seraye ne livraient pas des règles de gestion toutes faites : une phrase comme « il faudrait qu'on sache qui a vraiment participé » a dû être reformulée, question après question, jusqu'à devenir RG-15 à RG-18 (qui peut laisser un avis, à quel moment, une seule fois, avec quelle échelle de note). C'est cet exercice de reformulation, plus que l'écriture du code elle-même, qui a demandé le plus de rigueur : une règle mal comprise au départ se répercute silencieusement dans le diagramme de classes, puis dans le schéma SQL, puis dans chaque écran qui en dépend."),
  ...
  table(["Difficulté", "Solution mise en œuvre"], [
    ...
    ["Récupération de travail après une opération de migration automatisée ayant remisé (git stash) des modifications du backend non encore validées par un commit", "Inspection de git stash list et git reflog pour confirmer qu'aucune donnée n'était réellement perdue, puis restauration fichier par fichier et adoption d'un rythme de commit plus fréquent"],
    ...
    ["Cohérence stricte entre le diagramme de classes validé et les entités JPA réellement implémentées", "Vérification systématique des relations et des contraintes directement dans le code source (annotations @ManyToOne, @JoinColumn, @Table) et dans la base elle-même, plutôt que de faire confiance à la seule mémoire du schéma ; le code, le dictionnaire de données et le script SQL sont alignés sur le diagramme validé, jamais l'inverse"],
    ...
    ["Adresse postale de Terra Sana incohérente entre le pied de page du site et le service de génération d'attestations PDF", "Recherche de toutes les occurrences de l'adresse dans le code backend (PdfService et EmailService) et harmonisation sur l'adresse réelle du siège"],
  ], [3550, 5800]),
  ...
  bullet("L'absence de suite de tests automatisés significative (JUnit côté backend, tests de composants côté frontend), le dépôt ne contenant que les tests d'amorçage générés par défaut par les outils d'initialisation, reste une lacune dans la démarche qualité, et un axe d'amélioration prioritaire si le module devait évoluer davantage. La vérification manuelle décrite en section 3.6.2, bien que systématique, ne protège pas contre une régression future introduite par inadvertance."),
  bullet("L'attribution automatique des photos d'événements et de projets, basée sur une correspondance de mots-clés, reste une heuristique et non une reconnaissance d'image réelle : elle couvre vingt-huit catégories d'événements et vingt-neuf catégories de projets, mais nécessite un filet de sécurité (URL manuelle) pour les sujets réellement inédits."),
  ...
  bullet("l'ajout d'une gestion de plusieurs administrateurs avec des droits différenciés, envisagée dès l'analyse (section 1.5) mais volontairement écartée du périmètre pour tenir les délais de la deuxième session ;"),
...
const glossaire = [
  ...
  table(["Terme", "Définition"], [
    ...
    ["JWT (JSON Web Token)", "Jeton signé numériquement, transmis à chaque requête pour prouver l'identité de l'utilisateur, sans que le serveur ait besoin de conserver une session en mémoire (authentification « stateless »)."],
