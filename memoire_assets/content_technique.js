...
const technique = [
  ...
  table(["Composant", "Technologie", "Justification"], [
    ...
    ["Envoi d'emails", "Spring Boot Starter Mail (Gmail SMTP)", "Confirmations, relances et notifications automatiques (RG-12, RG-13, RG-20)"],
    ["Génération de PDF", "iText7 7.2.5", "Attestations de participation et export de la liste des inscrits (RG-21, RG-22)"],
    ["Frontend", "React 19.2.4", "Continuité avec le site développé pendant le stage, composants réutilisables"],
  ...
  table(["Couche", "Classe type", "Responsabilité"], [
    ...
    ["Model", "Event, Registration, AppUser, Review", "Entités JPA associées aux tables MySQL"],
  ...
  p("Quatre mécanismes, moins visibles qu'un écran mais représentatifs des choix d'implémentation retenus, méritent d'être détaillés : la gestion centralisée des erreurs, l'attribution automatique d'une photo aux événements et projets, l'envoi asynchrone des emails et l'architecture retenue pour l'interface multilingue."),
  ...
  p("L'administrateur de Terra Sana ne dispose généralement pas d'une URL d'image sous la main au moment de créer un événement ou un projet. Plutôt que d'afficher une image par défaut identique pour toutes les activités, EventImageService (et son équivalent ProjectImageService) recherche des mots-clés dans le titre et la description de l'événement, et associe la première catégorie thématique correspondante à une photo Unsplash déjà vérifiée. Vingt-huit catégories sont couvertes côté événements (cuisine, distribution alimentaire, sport, formation, nettoyage, marché bio, etc.) et vingt-neuf côté projets."),
  ...
  p("Le détail qui mérite d'être souligné porte sur l'envoi de l'email de félicitations (RG-20) : la comparaison newLevel.ordinal() > oldLevel.ordinal() garantit que ce message n'est déclenché qu'en cas de progression effective. Sans cette vérification, un bénévole dont le niveau redescendrait d'ARGENT à BRONZE après une désinscription recevrait, par erreur, le même email de félicitations qu'une réelle promotion : un cas limite qui n'apparaît pas dans une lecture rapide de la règle de gestion, mais que le code met en évidence."),
  ...
  table(["Étape", "Écran / action", "Règle(s) de gestion associées"], [
    ...
    ["5", "Il reçoit un email automatique confirmant son inscription ou sa mise en attente", "RG-12"],
  ...
  p("L'onglet Inscriptions centralise, tous événements confondus, la validation manuelle exigée par RG-11 : chaque ligne affiche le bénévole, l'événement concerné et son statut, avec deux boutons Confirmer / Refuser directement disponibles pour toute inscription encore en attente (statut WAITING)."),
  ...
  p("Les pages Sponsors et Bénévolat complètent la vitrine institutionnelle : la première présente les partenaires qui soutiennent Terra Sana (Bruxelles Environnement, la Région de Bruxelles, la commune de Woluwe-Saint-Lambert), la seconde propose un formulaire de candidature (bénévole ou stagiaire) dont le message parvient à l'association par le même canal que le formulaire de contact, en complément de la création de compte décrite en section 3.4.2."),
  ...
  p("Conformément à RG-25, les bénévoles et l'administrateur reçoivent chacun un jeton signé avec un secret distinct (JwtUtil pour l'admin, VolunteerJwtUtil pour le bénévole) et une durée de vie propre : 24 heures pour l'administrateur, 7 jours pour le bénévole, qui revient consulter ses inscriptions sur plusieurs jours (RG-23). Le filtre JwtFilter tente successivement les deux JwtUtil lors de la validation d'une requête et assigne le rôle ROLE_ADMIN ou ROLE_VOLUNTEER en conséquence : le rôle n'est pas un claim porté par le jeton lui-même, il est déduit de la clé de signature qui a permis de le valider. Ce filtre, exécuté une seule fois par requête (OncePerRequestFilter), s'intercale avant le filtre d'authentification standard de Spring Security :"),
  codeBlock([
  ...
  table(["Scénario testé", "Résultat attendu", "Résultat constaté"], [
    ...
    ["Confirmer une inscription en attente alors que l'événement vient d'atteindre sa capacité maximale entre-temps", "Rejet explicite, aucun dépassement de capacité (section 2.3)", "Conforme"],
